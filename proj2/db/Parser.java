...
public class Parser {
    ...
    static final String REST  = "\\s*(.*)\\s*", // spaces + any chars + spaces
                                COMMA = "\\s*,\\s*",    // spaces + , + spaces
                                AND   = "\\s+and\\s+";  // 1 or more spaces + and + 1 or more spaces

    // Stage 1 syntax, contains the command name.
    static final Pattern CREATE_CMD = Pattern.compile("create table " + REST),
    ...
    static final Pattern CREATE_NEW  = Pattern.compile("(\\S+)\\s+\\(\\s*(\\S+\\s+\\S+\\s*" +
                                                "(?:,\\s*\\S+\\s+\\S+\\s*)*)\\)"),
                                 ...
                                 ATTRIBUTES  = Pattern.compile("(\\S+\\s\\S+" + "(?:,\\S+\\s\\S+)*)"),
                                 VALUES      = Pattern.compile("((?:'.+?'|[0-9]*[.]*[0-9]*)" +
                                                "(?:,'.+?'|,[0-9]*[.]*[0-9]*)*)");
